...
Este script:
1. Lee los 4 datasets (TALIS, Eurydice, UNESCO, Eurostat/DESI) desde CSV
2. Clasifica cada fila en una dimensión ISDE (D1-D5)
3. Crea un CSV "dimensiones" como diccionario de referencia
4. Guarda los archivos transformados (.csv) en la carpeta Dimensiones
================================================================================
"""
...
def procesar_talis():
    ...
    # Guardar como CSV (datos + dimensiones por separado)
    output_datos = OUTPUT_PATH / 'TALIS_2024_DIMENSIONES_datos.csv'
    output_dims = OUTPUT_PATH / 'TALIS_2024_DIMENSIONES_dimensiones.csv'
    talis.to_csv(output_datos, index=False, encoding='utf-8-sig')
    df_dimensiones.to_csv(output_dims, index=False, encoding='utf-8-sig')
    
    print(f"   ✓ Guardado: {output_datos.name}")
    print(f"   ✓ Guardado: {output_dims.name}")
...
def procesar_eurydice():
    ...
    # Leer archivo
    eurydice = pd.read_csv(BASE_PATH / 'Eurydice.csv', encoding='utf-8-sig')
    ...
    # Guardar como CSV (datos + dimensiones por separado)
    output_datos = OUTPUT_PATH / 'Eurydice_DIMENSIONES_datos.csv'
    output_dims = OUTPUT_PATH / 'Eurydice_DIMENSIONES_dimensiones.csv'
    eurydice.to_csv(output_datos, index=False, encoding='utf-8-sig')
    df_dimensiones.to_csv(output_dims, index=False, encoding='utf-8-sig')
...
def procesar_unesco():
    ...
    # Leer archivo
    unesco = pd.read_csv(BASE_PATH / 'UNESCO.csv', encoding='utf-8-sig')
    ...
    # Guardar como CSV (datos + dimensiones por separado)
    output_datos = OUTPUT_PATH / 'UNESCO_DIMENSIONES_datos.csv'
    output_dims = OUTPUT_PATH / 'UNESCO_DIMENSIONES_dimensiones.csv'
    unesco.to_csv(output_datos, index=False, encoding='utf-8-sig')
    df_dimensiones.to_csv(output_dims, index=False, encoding='utf-8-sig')
...
def procesar_eurostat():
    ...
    # Leer archivo(s) transformado(s) - busca CSVs que coincidan con el patrón
    eurostat_pattern = list(BASE_PATH.glob('EuroStatDESI_TRANSFORMADO2*.csv'))
    
    if not eurostat_pattern:
        print(f"   ⚠️  No se encontraron archivos CSV con patrón 'EuroStatDESI_TRANSFORMADO2*.csv' en {BASE_PATH}")
        print("   El script está preparado para procesarlo cuando esté disponible.")
        return None
    
    eurostat = pd.concat(
        [pd.read_csv(f, encoding='utf-8-sig') for f in sorted(eurostat_pattern)],
        ignore_index=True
    )
    ...
    # Guardar como CSV (datos + dimensiones por separado)
    output_datos = OUTPUT_PATH / 'EuroStatDESI_DIMENSIONES_datos.csv'
    output_dims = OUTPUT_PATH / 'EuroStatDESI_DIMENSIONES_dimensiones.csv'
    eurostat.to_csv(output_datos, index=False, encoding='utf-8-sig')
    clasificacion_eurostat.to_csv(output_dims, index=False, encoding='utf-8-sig')
...
def crear_resumen_global():
    ...
    # Guardar resumen como CSVs separados
    output_desc = OUTPUT_PATH / 'RESUMEN_DIMENSIONES_ISDE.csv'
    output_keywords = OUTPUT_PATH / 'RESUMEN_KEYWORDS_EUROSTAT.csv'
    df_descripcion.to_csv(output_desc, index=False, encoding='utf-8-sig')
    
    # Crear CSV con palabras clave de Eurostat
    keywords_rows = []
    ...
    df_keywords.to_csv(output_keywords, index=False, encoding='utf-8-sig')
    
    print(f"   ✓ Resumen guardado: {output_desc.name}")
    print(f"   ✓ Keywords guardado: {output_keywords.name}")
...
📁 Archivos generados en {OUTPUT_PATH}:
   • TALIS_2024_DIMENSIONES_datos.csv / _dimensiones.csv
   • Eurydice_DIMENSIONES_datos.csv / _dimensiones.csv
   • UNESCO_DIMENSIONES_datos.csv / _dimensiones.csv
   • EuroStatDESI_DIMENSIONES_datos.csv / _dimensiones.csv
   • RESUMEN_DIMENSIONES_ISDE.csv
   • RESUMEN_KEYWORDS_EUROSTAT.csv

📋 Por cada dataset se generan dos CSVs:
   • *_datos.csv: Dataset original + columnas Dimension_ISDE y Dimension_Nombre
   • *_dimensiones.csv: Diccionario de referencia con la clasificación
