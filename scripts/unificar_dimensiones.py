# -*- coding: utf-8 -*-
"""
Unifica las hojas 'Datos' de varios Excel + un CSV grande en un único dataset.

- Lee:
  * Eurydice_DIMENSIONES.xlsx (sheet: Datos)      -> Estudio = "Eurydice"
  * TALIS_2024_DIMENSIONES.xlsx (sheet: Datos)   -> Estudio = "TALIS_2024"
  * UNESCO_DIMENSIONES.xlsx (sheet: Datos)       -> Estudio = "UNESCO"
  * EuroStatDESI_DIMENSIONES.csv                 -> Estudio = "EuroStat"

- Guarda:
  * DIMENSIONES_UNIFICADAS.csv      (UTF-8-SIG)
  * DIMENSIONES_UNIFICADAS.parquet  (si hay pyarrow)
...
SOURCES = [
    {
        "path": r"C:\Users\Usuario\Desktop\UNIR\TFM\Tablas\Dimensiones\Eurydice_DIMENSIONES.xlsx",
        "type": "excel",
        "sheet": "Datos",
        "estudio": "Eurydice",
    },
    {
        "path": r"C:\Users\Usuario\Desktop\UNIR\TFM\Tablas\Dimensiones\TALIS_2024_DIMENSIONES.xlsx",
        "type": "excel",
        "sheet": "Datos",
        "estudio": "TALIS",
    },
    {
        "path": r"C:\Users\Usuario\Desktop\UNIR\TFM\Tablas\Dimensiones\UNESCO_DIMENSIONES.xlsx",
        "type": "excel",
        "sheet": "Datos",
        "estudio": "UNESCO",
    },
    {
        "path": r"C:\Users\Usuario\Desktop\UNIR\TFM\Tablas\Dimensiones\EuroStatDESI_DIMENSIONES.csv",
        "type": "csv",
        "estudio": "EuroStat",
    },
]
...
def excel_header(path: str, sheet: str) -> List[str]:
    try:
        df0 = pd.read_excel(path, sheet_name=sheet, nrows=0, engine="openpyxl")
        return [str(c).strip().replace("\ufeff", "") for c in df0.columns]
    except ValueError as e:
        raise ValueError(f"No se encontró la hoja '{sheet}' en {path}.") from e
...
def compute_union_columns(sources: list) -> List[str]:
    ...
    for s in sources:
        p = s["path"]
        if s["type"] == "excel":
            headers = excel_header(p, s["sheet"])
        elif s["type"] == "csv":
            headers = csv_header(p)
        else:
            raise ValueError(f"Tipo no soportado: {s['type']}")
...
def main() -> None:
    ...
    for s in SOURCES:
        ...
        if s["type"] == "excel":
            df = pd.read_excel(p, sheet_name=s["sheet"], engine="openpyxl")
            
            # --- TRANSFORMACIONES ---
            df = transformar_datos(df)
            
            # Si tras filtrar el excel se queda vacío, pasamos al siguiente
            if df.empty:
                print(f"[INFO] Filas añadidas ({estudio}): 0 (filtradas todas)")
                continue

            df.insert(0, "Estudio", estudio)
            df = df.reindex(columns=union_cols)

            df.to_csv(OUTPUT_CSV, mode="a", index=False, header=not csv_header_written, encoding="utf-8-sig")
            csv_header_written = True

            if parquet_enabled and parquet_writer is not None:
                parquet_writer.write(df)

            print(f"[INFO] Filas añadidas ({estudio}): {len(df):,}")
